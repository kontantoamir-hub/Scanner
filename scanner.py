...
def atr_percent(ind, period=14):
    ...
    for i in range(n - period, n):
        ...
        trs.append(tr)
    return (sum(trs) / len(trs)) / ind["closes"][-1] * 100
...
def analyze_symbol(t, interval):
    ...
    try:
        ...
        # خطة دخول (شراء فقط — السوق الفوري لا يدعم فتح صفقة بيع مكشوفة)، تُحسب فقط عند توافق إيجابي كافٍ
        entry = sl = tp1 = tp2 = tp3 = None
        if final_score >= 1:
            recent_high = max(ind["highs"][-14:])
            recent_low = min(ind["lows"][-14:])
            price_range = recent_high - recent_low
            entry = ind["closes"][last]
            sl = entry - price_range * 0.35
            risk = entry - sl
...
def format_alert(r):
    ...
    if r.get("entry") is not None:
        ...
        lines.append(f"TP1: {r['tp1']:.6g}")
        lines.append(f"TP2: {r['tp2']:.6g}")
        lines.append(f"TP3: {r['tp3']:.6g}")
...
# ---------------- إدارة حالة التنبيهات (تفادي التكرار) ----------------
...
def main():
    ...
    # الاحتفاظ بذاكرة الإشارات الحالية فقط (لو اختفت عملة من القائمة القوية، ينفتح المجال لتنبيه جديد لاحقًا)
    save_state(strong_symbols)
    print("انتهى المسح.")
