...
EXCLUDE_SYMS = {
    ...
    # --- قائمة سوداء جديدة (19 عملة صفرية النجاح) ---
    "1000CATUSDT",
    "ADAUSDT",
    "ALGOUSDT",
    "ALLOUSDT",
...
def analyze_symbol(t, interval, error_list=None):
    ...
    try:
        ...
        # تعزيز وزن Squeeze في درجة الإشارة الرسمية (2026-09-14): بما أن الفلتر أعلاه لا
        # يسمح لـsqueeze=True إلا لو trend_up وmacd_bull متحققين معًا (أي الاتجاه صاعد
        # مؤكد أصلاً)، إضافة الوزن هنا مباشرة بدون مضاعف اتجاه — Squeeze بمفرده (بدون
        # Accumulation) يُعتبر مؤشرًا أقوى على حركة انفجارية وشيكة، فيُعطى وزنًا أعلى من
        # حالة تزامنه مع Accumulation (الذي يدل على تجميع هادئ بطيء لا انفجارًا سريعًا).
        # درجة منفصلة قبل بونص Squeeze (2026-09-15): final_score بعد إضافة البونص تُستخدم
        # لعتبات الإشارة الرسمية (سطر ~1006 وما بعده) وتبقى كما هي في حقل "score" المُرجَع.
        # لكن فلتر أهلية الإشارة المبكرة (early_eligible) كان يقارن بنفس final_score < 1.5
        # لتمييز "لم تصل بعد لإشارة رسمية" — وبما أن بونص Squeeze يرفع final_score مباشرة،
        # كانت عملات ذات Squeeze تتجاوز 1.5 بسبب البونص نفسه فتخرج من كلا الفلترين (لا رسمية
        # لعدم تحقق باقي شروط الرسمية، ولا مبكرة لتجاوز score الحد). نحفظ هنا القيمة قبل
        # البونص لتُستخدم حصرًا في بوابة early_eligible، بينما final_score/"score" الرسمية
        # تبقى بما فيها البونص كما كانت.
        ...
        if squeeze or accumulation:
            ...
            if conditions_met >= 1:
                ...
                if conditions_met >= 3:
                    early_tp_count = conditions_met + 1  # ثقة عالية (مؤكدة قوية) -> هدف إضافي
        ...
        return {
            ...
            "early_confidence": early_confidence, "early_target_capped": early_target_capped,
...
def main():
    ...
    early_eligible = [
        r for r in results
        if r["early_gate_score"] < 1.5 and (r["squeeze"] or r["accumulation"])
        and r.get("early_confidence") is not None
        and meets_min_profit(r["early_entry"], r["early_tps"])
        # الهدف الأخير للمبكرة أحيانًا يُقلَّم عند أقرب مقاومة (سقف سعري لا علاقة له
        # بمضاعفات R) — فحص R:R بهاي الحالة يرفض الإشارة رياضيًا بلا معنى حقيقي، لذا
        # يُستثنى فقط لو الهدف انقصّ بسبب مقاومة (early_target_capped)؛ باقي الأنواع
        # الثلاثة (رسمية/انفجار/تجريبية) تبقى خاضعة لفلتر R:R كالمعتاد بدون استثناء.
        and (r.get("early_target_capped") or meets_min_rr(r["early_entry"], r["early_sl"], r["early_tps"]))
    ]
